...
const calculer = () => {
  ...
  if (imc2 < 18.5) {
    ...
    const kilo2 = objectif2 - parseFloat(poids.value)
    info.value.kilo = kilo2.toFixed(1)
    info.value.classe ='alert-danger'
  } else if (imc2 < 25) {
    // 18.5 < imc < 25
    info.value.tranche = "normale"
    info.value.classe ='alert-success'
  } else if (imc2 < 30) {
    info.value.tranche = "surpoids"
    info.value.classe ='alert-warning'
  } else if (imc2 < 35) {
    info.value.tranche = "obésité"
     info.value.classe ='alert-danger'
  } else if (imc2 < 40) {
    info.value.tranche = "obésité massive"
    info.value.classe ='alert-secondary'
  } else if (imc2 > 40) {
    info.value.tranche = "obésité morbide"
    info.value.classe ='alert-primary'
  }
  poids.value = "";
  taille.value = "";
};
